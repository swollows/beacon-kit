...
def start(persistent_peers, is_seed, validator_index, config_settings, app_settings, kzg_impl):
    ...
    set_config += '\nsed -i \'s/^# other sinks such as Prometheus\\./# other sinks such as Prometheus./\' {}/config/app.toml'.format("$BEACOND_HOME")
    set_config += '\nsed -i \'s/^enabled = false$/enabled = true/\' {}/config/app.toml'.format("$BEACOND_HOME")
